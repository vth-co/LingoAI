...
async function generateQuestionsByAI(topic, native_language, level) {
...
Special people won.What is the adjective here ? using this` + prompt
        ...
        //family and friends, daily routines,shopping , food and drink (for everyday situations)
            ...
            prompt = role + `there're 3 levels :beginner, itermediate, advanced. Give me 3 unique Everyday Situations questions about  ${picked_topic} to test if  ${level} learner understand how to response or ask the question and 4 choices in english,answer and explaination in ${native_language}. using this` + prompt

        }
        else if (topic === "dailyroutines") {
            prompt = role + `there're 3 levels :beginner, itermediate, advanced. Give me 3 unique Daily Routines questions about  ${topic} to test if  ${level} learner understand how to response or ask the question and 4 choices in english,answer and explaination in ${native_language}. using this` + prompt

        }
        else if (topic === "shopping") {
            prompt = role + `there're 3 levels :beginner, itermediate, advanced. Give me 3 unique Shopping questions about  ${topic} to test if  ${level} learner understand how to response or ask the question and 4 choices in english,answer and explaination in ${native_language}. using this` + prompt

        }
        else if (topic === "foodanddrink") {
            prompt = role + `there're 3 levels :beginner, itermediate, advanced. Give me 3 unique Food and Drink questions about  ${topic} to test if  ${level} learner understand how to response or ask the question and 4 choices in english,answer and explaination in ${native_language}. using this JSON schema: { "type":"array", "properties": {"question": "string",  "options": "array",  "answer": "string",  "explanation":"string"}}.`
