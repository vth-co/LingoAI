...
async function generateVocabularyQuestionsByAI(topic, native_language, level) {
    ...
    // Default prompt
    let prompt = `You're an English teacher there're 3 student levels begineer, itermediate, advanced. Give me 3 unique fill-in-the-blank questions for ${topic} and 4 choices in english,answer and explaination in ${native_language} for ${level} learner using this JSON schema: { "type":"array", "properties": {"question": "string",  "options": "array",  "answer": "string",  "explanation":"string"}}.`
...
Don't eat the rotten apple. Identify the common noun in this sentence.
 using this JSON schema: { "type":"array", "properties": {"question": "string",  "options": "array",  "answer": "string",  "explanation":"string"}}.`
        ...
        prompt = `You're an English teacher there're 3 levels :begineer, itermediate, advanced. Give me 3 unique Basic Verbs questions to test if ${level} learner understand main verbs , auxiliaries and 4 choices in english,answer and explaination in ${native_language}. for example:
I usually listen _____ music when I'm on the bus to work using this JSON schema: { "type":"array", "properties": {"question": "string",  "options": "array",  "answer": "string",  "explanation":"string"}}.`
        ...
        prompt = `You're an English teacher there're 3 levels :begineer, itermediate, advanced. Give me 3 unique Basic Adjectives questions to test if ${level} learner understand main verbs , auxiliaries and 4 choices in english,answer and explaination in ${native_language}. for example:
Special people won.What is the adjective here ? using this JSON schema: { "type":"array", "properties": {"question": "string",  "options": "array",  "answer": "string",  "explanation":"string"}}.`
        ...
        prompt = `You're an English teacher there're 3 levels :begineer, itermediate, advanced. Give me 3 unique Common Phrases questions (Greetings, introductions, common questions (e.g., How are you?)) to test if ${level} learner understand how to response it and 4 choices in english,answer and explaination in ${native_language}. Using JSON schema: { "type":"array", "properties": {"question": "string",  "options": "array",  "answer": "string",  "explanation":"string"}}.`
    }
